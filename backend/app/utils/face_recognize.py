...
class Face_rec():
    ...
    def recognize(self, img, queried_features):
        stored_features = []
        stored_ids = []
        for queried_feature in queried_features:
            stored_features.append(np.frombuffer(queried_feature.feature, dtype=np.float32))
            stored_ids.append(queried_feature.s_id)

        # 提取人脸特征
        face_features = self.get_feature(img)
        ...
        for feature in face_features:
            ...
            if matches[best_match_index]:
                recognized_ids.append(stored_ids[best_match_index])
            else:
                recognized_ids.append("陌生人")
